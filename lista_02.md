...
# Resposta questão 7 (em pseudocódigo) :

### 7) Sistema de Classificação de Pedidos

```pseudo
funcao calcularFrete(valorTotal):
    se valorTotal < 50 entao
        retornar "Frete não disponível!"
    senao se valorTotal <= 199.99 entao
...
# Resposta questão 8 (em pseudocódigo):

### 8) Modelagem de Veículos - Classes Carro e Moto
...
classe Veiculo:
    ...
    metodo calcularConsumo():
        retornar "Método deve ser sobrescrito nas subclasses"

classe Carro herda Veiculo:
    atributo eficiencia  # km por litro
...
# Resposta questão 9 (em pseudocódigo): 

### 9) Cálculo de Tempo para Pouso de Sonda em Marte

```pseudo
funcao calcularTempoPouso(velocidadeInicial, desaceleracao, tempoMaximo, velocidadeSegura):
    tempo <- 0
    enquanto velocidadeInicial > velocidadeSegura e tempo < tempoMaximo:
        velocidadeInicial <- velocidadeInicial - desaceleracao
        tempo <- tempo + 1
    se velocidadeInicial <= velocidadeSegura entao
        retornar "Pouso seguro em " + tempo + " segundos."
    senao
        retornar "Tempo máximo excedido. Risco de desvio orbital!"
```
...
# Resposta questão 10 (em pseudocódigo):

### 10) Multiplicação de Matrizes de Investimento

```pseudo
funcao MultiplicarMatrizesInvestimento(matrizA, matrizB):
    se colunas(matrizA) ≠ linhas(matrizB) entao:
        retornar "As matrizes não podem ser multiplicadas. Dimensões incompatíveis."
    senao:
        matrizResultado <- novaMatriz(linhas(matrizA), colunas(matrizB))
        para i de 0 ate linhas(matrizA) - 1 faca:
            para j de 0 ate colunas(matrizB) - 1 faca:
                soma <- 0
                para k de 0 ate colunas(matrizA) - 1 faca:
